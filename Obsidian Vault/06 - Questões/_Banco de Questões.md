---
tipo: banco-questoes
criado: 2026-04-24
tags: [questões, revisão, prova]
---

# Banco de Questões

## Simulados criados

| Simulado | Objetivo | Questões | Link |
|---|---:|---:|---|
| Simulado 01 - Diagnóstico | Medir nível geral no formato do edital | 35 | [[Simulado 01 - Diagnóstico]] |
| Simulado 02 - Conhecimentos Específicos | Treinar o bloco de maior peso | 30 | [[Simulado 02 - Conhecimentos Específicos]] |
| Simulado 03 - Reta Final | Revisar pegadinhas e pontos críticos | 35 | [[Simulado 03 - Reta Final]] |

## Como usar

- Registrar cada questão errada ou duvidosa com link para a nota atômica relacionada.
- Marcar o padrão da falha: conceito, pegadinha, confusão entre alternativas ou leitura apressada.
- Ao errar duas vezes o mesmo assunto, criar ou reforçar flashcards.
- Corrigir pelo [[Gabaritos Comentados]] e lançar o resultado na [[Matriz de Desempenho]].
- Planejar a sequência pelo [[Plano de Simulados]].
- Encerrar cada aplicação com o [[Checklist Pós-Simulado]].

## Registro Rápido

| Disciplina | Tópico | Erro | Correção | Link |
|-----------|--------|------|----------|------|
| Trânsito |  |  |  |  |
| CIE |  |  |  |  |
| CBAE |  |  |  |  |
| IG |  |  |  |  |
| Português |  |  |  |  |

## Qualidade dos gabaritos

Auditoria de 2026-04-29: distribuição A=B=C=D=E=20% (antes: B=56%).  
Para re-auditar: `python3 scripts/auditoria_gabaritos.py --audit` (exit 0 = OK).  
Ver [[Auditoria de Gabaritos e Qualidade das Questões]] para histórico completo.

## Conexões

- [[Auditoria de Gabaritos e Qualidade das Questões]]
- [[_Índice de Erros]]
- [[Pegadinhas Fucap]]
- [[Resumo Brutal - Trânsito]]
- [[Gabaritos Comentados]]
- [[Matriz de Desempenho]]
- [[Plano de Simulados]]
- [[Checklist Pós-Simulado]]
